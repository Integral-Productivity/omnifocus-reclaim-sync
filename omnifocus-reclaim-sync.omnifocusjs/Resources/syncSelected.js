...
var action = new PlugIn.Action(async function (selection, sender) {
  try {
    ...
    // Check for quiet mode — set when invoked automatically by another action
    var quietSync = _prefs.read('quietSync') === true;
    if (quietSync) { _prefs.write('quietSync', false); }

    if (tasks.length === 0) {
      var none = new Alert('Sync Selected', 'No eligible tasks are selected.');
      none.addOption('OK');
      await none.show();
      return;
    }
    ...
    // Tell the user sync has started — skipped in quiet (auto-sync) mode since
    // the triggering action already gave feedback
    if (!quietSync) {
      _notify('Sync Selected', 'Syncing ' + tasks.length + ' task' + (tasks.length !== 1 ? 's' : '') + ' with Reclaim\u2026');
    }
